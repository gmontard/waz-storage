  ...
  module Blobs
    ...
    class Service
      ...
      def create_container(container_name)
        execute :put, container_name, {:restype => 'container'}, {:x_ms_version => '2012-02-12'}
      end
      
      # Retrieves all the properties existing on the container.
      def get_container_properties(container_name)
        execute(:get, container_name, {:restype => 'container'}, {:x_ms_version => '2012-02-12'}).headers
      end
      ...
      def set_container_properties(container_name, properties = {})
        execute :put, container_name, { :restype => 'container', :comp => 'metadata' }, properties.merge!({:x_ms_version => '2012-02-12'})
      end
      ...
      def get_container_acl(container_name)
        headers = execute(:get, container_name, { :restype => 'container', :comp => 'acl' }, {:x_ms_version => '2012-02-12'}).headers
      ...
      def set_container_acl(container_name, public_available = WAZ::Blobs::BlobSecurity::Private)
        publicity = {:x_ms_version => '2012-02-12' }
      ...
      def delete_container(container_name)
        execute :delete, container_name, {:restype => 'container'}, {:x_ms_version => '2012-02-12'}
      end

      # Lists all the blobs inside the given container.
      def list_blobs(container_name)
        content = execute(:get, container_name, { :restype => 'container', :comp => 'list'}, {:x_ms_version => '2012-02-12'})
        doc = REXML::Document.new(content)
        containers = []
      ...
      def put_blob(path, payload, content_type = "application/octet-stream", metadata = {})
        default_headers = {"Content-Type" => content_type, :x_ms_version => "2012-02-12", :x_ms_blob_type => "BlockBlob", :x_ms_meta_railsetag => Digest::MD5.hexdigest(payload)}
      ...
      def put_block_list(path, blockids, content_type = "application/octet-stream", metadata = {})
        default_headers = {"Content-Type" => content_type, :x_ms_version => "2012-02-12"}
      ...
      def snapshot_blob(path)
        execute(:put, path, { :comp => 'snapshot' }, {:x_ms_version => "2012-02-12"}).headers[:x_ms_snapshot]
      end
    end
  end
end
