  ...
  module Queues
    ...
    class Service
      ...
      def list_queues(options ={})
        content = execute(:get, nil, { :comp => 'list' }.merge!(options), { :x_ms_version => "2012-02-12" })
      ...
      def create_queue(queue_name, metadata = {})
        execute(:put, queue_name, nil, metadata.merge!(:x_ms_version => '2012-02-12'))
      end
      
      # Deletes the given queue from the current storage account.
      def delete_queue(queue_name)
        execute(:delete, queue_name, {}, {:x_ms_version => '2012-02-12'})
      end
      
      # Gets the given queue metadata.
      def get_queue_metadata(queue_name)
        execute(:head, queue_name, { :comp => 'metadata'}, :x_ms_version => '2012-02-12').headers
      end
      
      # Sets the given queue metadata.
      def set_queue_metadata(queue_name, metadata = {})
        execute(:put, queue_name, { :comp => 'metadata' }, metadata.merge!(:x_ms_version => '2012-02-12'))
      end
      ...
      def enqueue(queue_name, message_payload, ttl = 604800)
        ...
        execute(:post, "#{queue_name}/messages", { :messagettl => ttl }, { 'Content-Type' => 'application/xml', :x_ms_version => "2012-02-12"}, payload)
      end
      ...
      def get_messages(queue_name, options = {})
        ...
        content = execute(:get, "#{queue_name}/messages", options, {:x_ms_version => "2012-02-12"})
        doc = REXML::Document.new(content)
        messages = []
      ...
      def clear_queue(queue_name)
        execute :delete, "#{queue_name}/messages", {}, :x_ms_version => '2012-02-12'
      end
    end
  end
end